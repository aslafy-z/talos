...
type Collector interface {
	Collect(devPath, transport string) Result
}
...
func (c *RealCollector) Collect(devPath, transport string) Result {
	source := sourceFromTransport(transport)
	if source == block.DiskHealthSourceUnsupported {
		return Result{
			Source: block.DiskHealthSourceUnsupported,
			Status: block.DiskHealthStatusValueUnknown,
			Error:  fmt.Sprintf("transport %q does not support health monitoring", transport),
		}
	}
...
func sourceFromTransport(transport string) block.DiskHealthSource {
	switch transport {
	case "nvme":
		return block.DiskHealthSourceNVME
	case "ata", "sata":
		return block.DiskHealthSourceATA
	default:
		return block.DiskHealthSourceUnsupported
	}
}
...
// Pending sectors, reallocated sectors, and reported uncorrectable errors
// are degradation warnings (cumulative counters, like NVMe media errors).
func ComputeATAStatus(d *block.DiskHealthATADetails) block.DiskHealthStatusValue {
	if d.OfflineUncorrectableCount > 0 {
		return block.DiskHealthStatusValueCritical
	}

	if d.CurrentPendingSectorCount > 0 || d.ReallocatedSectorCount > 0 || d.ReportedUncorrectableErrors > 0 {
		return block.DiskHealthStatusValueWarning
	}
