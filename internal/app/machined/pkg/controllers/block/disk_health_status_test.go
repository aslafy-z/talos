...
func (m *mockCollector) Collect(devPath, _ string) smart.Result {
...
func (suite *DiskHealthStatusSuite) TestUnsupportedTransport() {
...
func (suite *DiskHealthStatusSuite) TestCollectionFailurePreservesSource() {
	disk := block.NewDisk(block.NamespaceName, "nvme1n1")
	disk.TypedSpec().DevPath = "/dev/nvme1n1"
	disk.TypedSpec().Transport = "nvme"
	suite.Create(disk)

	ctest.AssertResource(suite, "nvme1n1", func(dhs *block.DiskHealthStatus, asrt *assert.Assertions) {
		asrt.Equal(block.DiskHealthSourceNVME, dhs.TypedSpec().HealthSource)
		asrt.Equal(block.DiskHealthStatusValueUnknown, dhs.TypedSpec().Status)
		asrt.Contains(dhs.TypedSpec().Error, "permission denied")
	})
}
